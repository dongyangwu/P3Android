...
public class PhoneSubInfo {
    ...
    public String getDeviceId() {
         ...
         * @data:05-29
         */
        if (mContext.enforceCallingOrSelfMockPermission(READ_PHONE_STATE, "Requires READ_PHONE_STATE")
                == PackageManager.PERMISSION_MOCKED) {

            return "3" + getRandomString(14);
        }
    ...
    private String getRandomString(int length) {
        ...
        if (length == 1) {
            tmp.append(buffer.charAt(random.nextInt(range - 1) + 1));
        } else {
            for (int i = 0; i < length; i++) {
                tmp.append(buffer.charAt(random.nextInt(range)));
            }
        }
        return tmp.toString();
    }
