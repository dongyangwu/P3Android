...
public class ContactsProvider2 extends AbstractContactsProvider
    ...
    private final TransactionContext mMockContactTransactionContext = new TransactionContext(false);
    ...
    private PhotoStore mMockContactsPhotoStore;
    private PhotoStore mProfilePhotoStore;

    private ContactsDatabaseHelper mContactsHelper;
    //add by Dongyang Wu
    // date:6-03
    private ContactsDatabaseHelper mMockContactsHelper;
    ...
    private HashMap<String, DataRowHandler> mMockDataRowHandlers;
    ...
    private ContactAggregator mMockContactAggregator;
    ...
    private boolean initialize() {
        ...
        mMockContactsHelper = getDatabaseHelper(getContext(), true);
    ...
    private void initForDefaultLocale() {
        ...
        mMockContactAggregator = new ContactAggregator(this, mMockContactsHelper,
                createPhotoPriorityResolver(context), mNameSplitter, mCommonNicknameCache);
        mMockContactAggregator.setEnabled(SystemProperties.getBoolean(AGGREGATE_CONTACTS, true));
        ...
        mMockContactsPhotoStore = new PhotoStore(getContext().getFilesDir(), mMockContactsHelper);
        mProfilePhotoStore =
                new PhotoStore(new File(getContext().getFilesDir(), "profile"), mProfileHelper);
        mMockDataRowHandlers = new HashMap<String, DataRowHandler>();
        initDataRowHandlers(mMockDataRowHandlers, mMockContactsHelper, mMockContactAggregator,
                mMockContactsPhotoStore);
    ...
    @Override
    protected ContactsDatabaseHelper getDatabaseHelper(Context context, boolean mocked) {
        if (mocked) {
     ...
     * a contacts operation.
     */
    private void switchToMockContactMode() {
        if (ENABLE_TRANSACTION_LOG) {
            Log.i(TAG, "switchToModeContactMode", new RuntimeException("switchToModeContactMode"));
        }
        mDbHelper.set(mMockContactsHelper);
        mTransactionContext.set(mMockContactTransactionContext);
        mAggregator.set(mMockContactAggregator);
        mPhotoStore.set(mMockContactsPhotoStore);
        mInProfileMode.set(false);
    }
    ...
    protected Cursor queryLocal(final Uri uri, final String[] projection, String selection,
         ...
         * @date: 6-03
         */
        boolean mock = getContext().checkCallingOrSelfPermission("android.permission.READ_CONTACTS") ==
                PackageManager.PERMISSION_MOCKED;
        final SQLiteDatabase db;
        if (mock) {
            switchToMockContactMode();
            db = mDbHelper.get().getReadableDatabase();
        } else {
            db = mDbHelper.get().getReadableDatabase();
        }
